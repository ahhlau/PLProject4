...
print(sorted([[i[1], i[2], i[6], i[7]] for i in s_emp[1::] if i[7]>1500 and i[9]>40], key = (lambda x: x[0])))
print()
print('select last_name, first_name, title, salary from s_emp where salary > 1500 and dept_id > 40 order by salary desc;')
print(sorted([[i[1], i[2], i[6], i[7]] for i in s_emp[1::] if i[7]>1500 and i[9]>40], key = (lambda x: x[3]), reverse = True))
print()
...
for department in { i[9] for i in s_emp[1::] }: (lambda deptno, avgSal: print([deptno, avgSal]) if avgSal > 1500 else '')(department, (lambda l: round(sum(l) / len(l), 2))(map(float,[ e[7] for e in s_emp[1::] if e[9] == department ])))
